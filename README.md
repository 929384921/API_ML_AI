...
### 人工智能概率性（精确度）
- 菜谱API中有大量的菜谱数据，精确度较高，**达到95%以上**；但不能确定罕见的同一菜品若输入不同叫法是否能够完全准确的找到对应的菜谱。(至少番茄炒蛋和西红柿炒蛋在尝试中没有问题)
- 科大讯飞的语音合成API虽然在大段的文字识别中有较高的准确度，**达到98%以上**，但当文本的标点符号缺失或文本过长时有可能出现语音短句出现误差的情况。
